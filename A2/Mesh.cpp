...
Mesh::decimateQuadricEdgeCollapse()
{
  ...
  double minErrorTillNow = 1000000;
  for(auto it = edgesBegin(); it!= edgesEnd(); it++){
    Edge* temp = &(*it);
    double errorNow = temp->getQuadricCollapseError();
    if(errorNow>0 && errorNow<minErrorTillNow){
      minErrorTillNow = errorNow;
      requiredEdge = temp;
    }
  }

  // (2) Collapse the min error edge, if a valid one is found 
  // (else return NULL). 
  // There is a convenient function for this (it is
  // a good exercise to study how this function operates). 
  // Make a note of the single vertex which is the result of the
  // collapse.
  if(requiredEdge==NULL)
    return NULL;

  Vertex* newVertex = requiredEdge->getEndpoint(0);
  Vector3 newPosition = requiredEdge->getQuadricCollapsePosition();
  collapseEdge(requiredEdge);
  newVertex->setPosition(newPosition);

  // (3) Update the normal of every face incident on this vertex.
  for(auto it = newVertex->facesBegin(); it != newVertex->facesEnd(); it++){
    (*it)->updateNormal();
  }  
  ...
  newVertex->updateQuadric();
  newVertex->updateNormal();
  
  // (5) For every edge incident on this vertex:
  // - Update the quadric and normal for its other endpoint, 
  // which has also been affected.
  // - Update the quadric collapse error and 
  // the optimal collapse position for the edge.
  for(auto it = newVertex->edgesBegin(); it!=newVertex->edgesEnd(); it++){
    Vertex* otherVertex = (*it)->getOtherEndpoint(newVertex);
    otherVertex->updateQuadric();
    otherVertex->updateNormal();
